...
void CNetwork::DisplayText(const char* fmt, ...)
{
	...
	//EnterCriticalSection(&cnetwork->cs);
	//int nLength = GetWindowTextLength(cnetwork->hEdit);
	//SendMessage(cnetwork->hEdit, EM_SETSEL, nLength, nLength);
	//SendMessageA(cnetwork->hEdit, EM_REPLACESEL, FALSE, (LPARAM)cbuf);
	//LeaveCriticalSection(&cnetwork->cs);
}
